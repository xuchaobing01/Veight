...
module.exports = app => {
   ...
   * 购车计划事件服务
   */
  class BuyCarToSalemanService extends app.Service {
    ...
    * getDetailType(context) {
      ...
      if (cur_executor && cur_executor.extend && cur_executor.extend.is_order && cur_executor.extend.expert) {
        events.push(Events.detail.buy_car_to_saleman_iweek);
      }
    ...
    * getEventTask(event_rules, context) {
      ...
      const event_tasks = event_rules.map(e_v => {
        ...
        if (e_v.event_type === Events.detail.buy_car_to_saleman_iweek) {
          e_t.related = {
            id: context.task._id.toString(),
            func_model: Events.category.buy_car_to_saleman,
          };
          ...
          rule.content = e_v.content.replace(/\${bee}/, cur_executor.user.name);
        }

        if (e_v.task_type === Events.taskType.system.buy_car_invitation_entry_shop && task.extend) {
